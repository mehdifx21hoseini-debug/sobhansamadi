		(function () {
			...
			// موتور سشن از lib/sessions.js می‌آید - همان کد، فقط جای دیگر.
			// این نام‌های محلی می‌مانند تا بقیه‌ی این فایل دست‌نخورده بماند:
			// ۵۵ جای دیگر صدایشان می‌زند و عوض کردنِ همه‌ی آن‌ها در یک قدم،
			// تغییری که قرار بود بی‌اثر باشد را پرریسک می‌کرد.
			var SESSIONS = MarketSessions.SESSIONS;
			var sessionByKey = MarketSessions.sessionByKey;
			var inDailyBreak = MarketSessions.inDailyBreak;
			var breakEndAfter = MarketSessions.breakEndAfter;
			var breakStartAfter = MarketSessions.breakStartAfter;
			var tzParts = MarketSessions.tzParts;
			var tzOffsetMinutes = MarketSessions.tzOffsetMinutes;
			var instantAt = MarketSessions.instantAt;
			var localDatePlus = MarketSessions.localDatePlus;
			var occurrences = MarketSessions.occurrences;
			var sessionState = MarketSessions.sessionState;
			var marketState = MarketSessions.marketState;
			var hhmmInZone = MarketSessions.hhmmInZone;
