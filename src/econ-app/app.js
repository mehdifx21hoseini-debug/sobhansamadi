		(function () {
			...
			function renderHero() {
				...
				if (todays.length === 0) {
					...
					// نامِ کوتاه، نه کامل.
					//
					// e.en همان enFull سرور است و نامِ خامِ منبع را می‌دهد -
					// گاهی صد کاراکتر: «GDP (Third Estimate), Industries,
					// Corporate Profits, State GDP, and State Personal
					// Income, 2nd Quarter 2026; State PCE, 2025». این یک
					// خطِ خلاصه است و آن نام سه خطش می‌کرد. e.short از
					// سرور می‌آید و از پیش روی ۴۰ کاراکتر بریده شده؛ نامِ
					// کامل سرِ جایش هست و با باز کردنِ خودِ خبر دیده می‌شود.
					if (next) sub += " — بعدی " + relDay(next.date) + "، " + (next.short || next.title || "");
				} else {
			...
			function eventNode(e) {
				...
				if (e.read) {
					...
					// «تأثیر معمول» - همان چیزی که فارکس‌فکتوری Usual Effect
					// می‌نامد. تا امروز سرور این ستون را می‌فرستاد و اپ فقط
					// وقتی نشانش می‌داد که عدد منتشر شده باشد (داخلِ e.read)،
					// یعنی دقیقاً وقتی دیگر لازم نبود.
					//
					// پیش از انتشار همان چیزی است که کاربر می‌خواهد بداند:
					// اگر عدد بالاتر درآمد، خوب است یا بد؟
					var ccy = String(e.currency || "USD").toUpperCase();
					var up = e.direction !== "inverse";
					detail.appendChild(el("div", "usual",
						"معمولاً عددِ " + (up ? "بالاتر" : "پایین‌تر") +
						" از پیش‌بینی برای " + ccy + " مثبت است"));
				}

				// «قبلی» که منبع بازنگری‌اش کرده.
				//
				// این هم از قبل می‌آمد و هیچ‌جا نوشته نمی‌شد. بازنگریِ عددِ
				// ماه پیش خودش خبر است: «۰٫۸٪ (بازنگری‌شده از ۱٫۱٪)» یعنی
				// تصویرِ قبلی خوش‌بینانه بوده، و بدونِ آن، فقط یک عددِ تازه
				// دیده می‌شود.
				if (e.previous_before && e.previous_before !== e.previous) {
					detail.appendChild(el("div", "revised",
						"قبلی از " + fa(e.previous_before) + " به " + fa(e.previous || "—") + " بازنگری شد"));
				}
			...
			function renderList() {
				...
				if (events.length === 0) {
					...
					// حالتِ خالی باید بگوید *چرا* خالی است و یک راه بدهد.
					//
					// «با این فیلتر چیزی نیست» درست بود ولی بی‌فایده: فیلتر
					// سه چیز است - ارز، سطحِ اهمیت، و بازه - و کاربر
					// نمی‌دانست کدامشان. بدتر اینکه انتخابِ ارز تا همین
					// امروز فقط در تبِ تنظیمات بود، پس راهِ برگشت در صفحه‌ی
					// دیگری بود و این صفحه اشاره‌ای به آن نمی‌کرد.
					//
					// ترتیبِ بررسی از محتمل‌ترین علت شروع می‌شود، و هر شاخه
					// دکمه‌ی خودش را می‌دهد نه یک دکمه‌ی عمومی.
					var sub = state.data.subscription;
					var chosen = (sub && sub.currencies && sub.currencies.length)
						? sub.currencies : ["USD"];
					var onlyOne = chosen.length === 1;
					var filtered = state.importance !== "all";
					...
					if (onlyOne) {
						empty.appendChild(document.createTextNode(
							"با " + chosen[0] + " تنها، " +
							(state.scope === "today" ? "امروز" : "این هفته") + " خبری نیست."));
						empty.appendChild(document.createElement("br"));
						// یورو و پوند: پرخبرترین ارزها بعد از دلار، و همان
						// دو تایی که تقریباً همیشه فهرست را پر می‌کنند.
						var add = ["EUR", "GBP"].filter(function (c) { return chosen.indexOf(c) < 0; });
						if (add.length) {
							var turn = el("button", "chip", "روشن کردن " +
								add.map(function (c) { return c === "EUR" ? "یورو" : "پوند"; }).join(" و "));
							turn.type = "button";
							turn.style.marginTop = "12px";
							turn.addEventListener("click", function () { addCurrencies(add); });
							empty.appendChild(turn);
						}
					} else if (filtered) {
						empty.appendChild(document.createTextNode(
							"با فیلترِ «" +
							(state.importance === "high" ? "خیلی مهم" : "مهم به بالا") +
							"»، " + (state.scope === "today" ? "امروز" : "این هفته") + " خبری نیست."));
						empty.appendChild(document.createElement("br"));
						var all = el("button", "chip", "نمایش همه‌ی خبرها");
						all.type = "button";
						all.style.marginTop = "12px";
						all.addEventListener("click", function () {
							state.importance = "all";
							haptic("select");
							document.querySelectorAll(".chip[data-imp]").forEach(function (b) {
								b.setAttribute("aria-pressed", b.dataset.imp === "all" ? "true" : "false");
							});
							renderList();
						});
						...
						empty.appendChild(document.createTextNode(
							state.scope === "today"
								? "امروز خبری ثبت نشده — روز آرامی است."
								: "در این هفته خبری ثبت نشده."));
					}

					// روزِ خالی در آخر هفته و تعطیلیِ بانکی طبیعی است، پس
					// راهِ «کلِ هفته» همیشه کنارِ بقیه می‌ماند.
					if (state.scope === "today" && state.data.events.length > 0) {
						var jump = el("button", "chip", "نمایش کل هفته");
						jump.type = "button";
						jump.style.marginTop = "12px";
						jump.style.marginInlineStart = "6px";
			...
			//
			// چرا جدا از toggleCurrency: صدا زدنِ پشتِ سرِ هم آن تابع کار
			// نمی‌کند. هر بار از روی sub.currencies حساب می‌کند و بینشان
			// یک ذخیره و یک بازخوانی می‌نشیند، پس دومی یا روی مقدارِ کهنه
			// می‌نشیند و اولی را پاک می‌کند، یا با state.saving بی‌صدا رد
			// می‌شود. با تایمر فاصله انداختن هم فقط شانس را بالا می‌برد،
			// حلش نمی‌کند.
			function addCurrencies(codes) {
				...
				codes.forEach(function (c) { if (list.indexOf(c) < 0) list.push(c); });
				...
				call({
					...
					.then(function (body) {
						...
						if (body && body.subscription) {
							...
							if (undo) {
								renderCurrencies();
								// فیلترِ ارز سمتِ سرور اعمال می‌شود - اپ
								// رویدادهای ارزهای خاموش را اصلاً دریافت
								// نمی‌کند. پس بدونِ این خواندنِ دوباره،
								// روشن کردنِ یورو هیچ خبری به فهرست اضافه
								// نمی‌کرد تا وقتی کاربر خودش بروزرسانی
