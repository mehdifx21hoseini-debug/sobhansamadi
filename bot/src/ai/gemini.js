...
//
// - مدل embedding و ابعادش حیاتی است: بردار سوال و بردار مدخل باید با یک
//   مدل و یک تعداد بُعد ساخته شده باشند، وگرنه شباهت کسینوسی عدد بی‌معنی
//   می‌دهد. برای همین هر تغییر در این دو عدد یعنی همه‌ی بردارهای ذخیره‌شده
//   باید دوباره ساخته شوند - این کار در source.js انجام می‌شود.
// - دمای ۱.۱ عمدی است: قانون ۳۷ پرامپت می‌خواهد پاسخ‌های تکراری هر بار با
//   جمله‌بندی تازه بیایند.
const EMBED_MODEL = "gemini-embedding-001";

// ۲۵۶ بُعد از نسخه‌ی n8n به ارث رسیده بود و برای این پایگاه دانش کم است:
// ۹۵ مدخلش درباره‌ی یک اکسپرت‌اند و متن‌شان به هم شبیه؛ در ۲۵۶ بُعد این‌ها
// عملاً روی هم می‌افتند و «Half Close» از «Close All» جدا نمی‌شود.
