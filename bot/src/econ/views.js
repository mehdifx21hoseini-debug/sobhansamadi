...
export function buildAlertSettingsText(sub) {
  ...
  return (
    "🔔 هشدار اخبار اقتصادی\n\n" +
    // پیامِ «عدد منتشر شد» برداشته شد - سه شاخصی که با هم منتشر می‌شوند سه
    // پیامِ پشتِ سرِ هم می‌ساختند. این متن هم باید با آن بخواند، وگرنه
 ...
 */
// نشانه‌گذارِ چپ‌به‌راست، و خطِ عمودیِ جداکننده‌ی ستون‌ها.
const LRM = "\u200e";
const COL = "\u2502";
...
function eventDetails(e, h, opts = {}) {
  ...
  // ساعت با رقمِ لاتین می‌ماند، برخلافِ بقیه‌ی عددهای ربات.
  //
  // دو دلیل: در فونتِ مونواسپیس رقم‌های لاتین واقعاً هم‌عرض‌اند و ستون را
  // صاف نگه می‌دارند، و خودِ کاربر هم ساعت را در متاتریدر و در هر تقویمِ
  // دیگری به همین شکل می‌بیند - «۱۶:۰۰» تنها جایی بود که فرق می‌کرد.
  //
  // این فقط ساعتِ ساعت‌وار است. مدت‌ها («۱۵ دقیقه دیگر») و تاریخ‌ها فارسی
  // می‌مانند؛ آن‌ها عددِ متن‌اند نه عددِ ستون.
  const t = e.time ? etTimeToTehran(e.date, e.time, "") : "-";
  ...
  // فقط شلوغی است.
  // خطِ خلاصه چپ‌به‌راست است، نه راست‌به‌چپ.
  //
  // این تنها راهِ ستون‌دار شدنش بود. با RLM جهتِ خط راست‌به‌چپ می‌شد و
  // ترتیبِ دیده‌شدن وارونه‌ی ترتیبِ نوشته‌شده: چیزی که اول می‌نوشتیم آخر
  // سمتِ راست می‌نشست. بدتر از آن، نامِ لاتین و ساعتِ فارسی و ایموجی سه
  // جنسِ متفاوتِ دوجهته‌اند و مرورگر برای هر ردیف بسته به محتوایش
  // تصمیمِ کمی متفاوتی می‌گرفت - برای همین فهرست «پشتِ سرِ هم» و
  // نامرتب دیده می‌شد.
  //
  // با LRM خطِ خلاصه یک ردیفِ ثابت است: ساعت، اهمیت، نام - از چپ، همان
  // ترتیبی که نوشته می‌شود. ساعت هم پنج نویسه و مونواسپیس است، پس
  // جداکننده‌ها در همه‌ی ردیف‌ها زیرِ هم می‌افتند.
  //
  // خطِ عمودی به‌جای فاصله: فاصله‌ی چندتایی در رندر جمع می‌شود و همان
  // «پشتِ سرِ هم» را می‌سازد؛ یک نویسه‌ی جداکننده جمع نمی‌شود و مرزِ
  // ستون را واقعاً نشان می‌دهد.
  const lines = [
    "<details><summary>" +
      LRM + "`" + mdCell(t) + "` " + COL + " " + emoji + " " + flag + COL + " " + mdCell(short) +
      "</summary>",
    "",
  ];
