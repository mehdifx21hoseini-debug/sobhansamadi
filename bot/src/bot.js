  ...
  bot.on("callback_query:data", async (ctx) => {
    ...
    if (data === "INTRO_REGISTER_ADVANCED") {
      ...
      await startRegistrationFlow(
        ...
        // فقط این ورودی انتخاب دوره را رد می‌کند؛ دکمه‌ی منوی اصلی هر سه
        // گزینه را نشان می‌دهد.
        { skipCourseChoice: true }
      );
      return;
    }
    ...
    if (data === "REAL_YES" || data === "REAL_NO") {
      await ctx.answerCallbackQuery();
      await handleRealChoice(ctx, data);
      return;
    }
