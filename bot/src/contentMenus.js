...
/**
 * «ذهن ثروتمند»: عکسِ جلد با متن زیرش، بعد ویدئو، بعد دکمه‌ی خرید.
 *
 * ترتیب عمدی است. کاربر اول جلد را می‌بیند و می‌خواند کتاب درباره‌ی
 * چیست، بعد نویسنده و کتاب را در ویدئو می‌بیند، و دکمه‌ی خرید آخر
 * می‌آید - وقتی دلیلی برای زدنش دارد. دکمه زیر ویدئو می‌نشیند نه زیر
 * متن، تا آخرین چیزی باشد که کاربر می‌بیند.
 *
 * عکس و متن یک پیام‌اند نه دو تا: جلد بدونِ توضیح نصفه است و توضیح
 * بدونِ جلد خشک. تلگرام کپشن را تا ۱۰۲۴ کاراکتر می‌پذیرد و متنِ کتاب
 * حدود ۷۸۵ است، ولی متن از ویرایشگر عوض می‌شود و ممکن است بلندتر شود -
 * آن وقت عکس و متن دو پیام می‌شوند، که از خطای تلگرام بهتر است.
 *
 * هیچ‌کدام از این دو اجباری نیست: بی‌عکس، متن تنها می‌رود؛ و اگر ویدئو
 * هنوز از کانال نیامده باشد، متن و دکمه سرِ جایشان می‌روند. نبودنِ یک
 * رسانه نباید کتاب را غیرقابل‌سفارش کند.
 */
async function sendOwnBook(ctx) {
  const section = await resolveSection(ctx.env, "BOOK_00");
  const body = section.text || D.BOOK_00_TEXT;
  ...
  if (photo) {
    try {
      if (body.length <= CAPTION_LIMIT) {
        await ctx.replyWithPhoto(photo, { caption: body });
      } else {
      ...
      console.error("ارسال عکس کتاب شکست خورد:", err && err.message);
    }
  }
  // عکس نبود یا نرفت - متن به‌هر‌حال باید برسد.
  if (!shown) await ctx.reply(body);
...
export async function handleExpertPlatform(ctx, platform) {
  ...
  // کتابچه‌ی راهنما، پیش از خودِ فایل.
  //
  // چرا جدا، وقتی داخلِ همان rar هست: کسی که فایل را روی گوشی می‌گیرد
  // معمولاً از حالت فشرده خارجش نمی‌کند - آن را به کامپیوتر می‌فرستد و
  // راهنما همان‌جا دیده‌نشده می‌ماند. جدا که برود، همان‌جا در تلگرام باز
  // می‌شود.
  //
  // و چرا پیش از فایل و نه بعدش: همان دلیلی که یادداشتِ بروکر پیش از
  // فایل می‌رود - آخرین پیامِ گفت‌وگو باید خودِ اکسپرت باشد، تا موقع
  // نصب لازم نباشد کاربر در چت بالا برود دنبالش.
  //
  // اگر هنوز در کانال گذاشته نشده، بی‌صدا رد می‌شود. کاربر راهنما را
  // نخواسته بود؛ گفتنِ «راهنما آماده نیست» او را نگرانِ چیزی می‌کند که
  // در همان آرشیو دارد.
  try {
    await deliverContent(ctx, "EXPERT_MANUAL_PDF", {
      extra: {
        caption:
          "📘 کتابچه‌ی راهنمای اکسپرت SsProX\n\n" +
          "همین فایل داخلِ آرشیو هم هست؛ اینجا جدا گذاشته شد تا لازم نباشد برای خواندنش آرشیو را باز کنید.",
      },
    });
  } catch (err) {
    console.error("کتابچه‌ی راهنما نرفت:", err && err.message);
  }
  ...
  try {
    ...
    // فایل بی‌دکمه می‌رود.
    //
    // پیش از این کیبوردِ اکسپرت زیرش می‌نشست تا «قدم بعدی» در دسترس
    // باشد. حالا قدمِ بعدی بالای فایل گفته شده - یادداشتِ بروکر با
    // دکمه‌ی خودش - و تکرارِ چهار دکمه زیرِ فایل فقط همان پیام را شلوغ
    // می‌کرد. کاربر در این لحظه یک کار دارد: برداشتنِ فایل.
    //
    // راهِ برگشت بسته نمی‌شود: کیبوردِ منوی اصلی همیشه پایین صفحه است.
  ...
  // جای‌گزینِ فایل وقتی هنوز آپلود نشده. این هم بی‌دکمه می‌رود، به همان
  // دلیل: جای فایل را گرفته، پس باید شبیه فایل رفتار کند.
  if (delivered === 0) {
    await ctx.reply(
