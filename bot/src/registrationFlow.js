...
// بی‌آن.
// نویسه‌ها فارسی‌اند، نه عربی.
//
// این رشته از یک متنِ کپی‌شده آمده بود و سه نویسه‌ی عربی داشت: «ي»
// به‌جای «ی» و رقمِ «١» به‌جای «۱». روی صفحه تفاوتش کم است - «ي» در
// پایانِ کلمه دو نقطه می‌گیرد - ولی برای هر چیزی که متن را مقایسه
// می‌کند دو حرفِ متفاوت‌اند: جست‌وجو، بازیابیِ پایگاه دانش، و هر
// تطبیقی که روی همین جمله بنشیند. بقیه‌ی ربات فارسی می‌نویسد؛ این
// یک خط بود که فرق می‌کرد.
export const NAME_QUESTION = "۱- نام و نام خانوادگی خودتون رو بفرمایید";
...
/**
 * کیبوردِ درخواستِ شماره، با متنِ راهنما داخلِ کادرِ تایپ.
 *
 * تلگرام به هیچ رباتی اجازه نمی‌دهد کادرِ تایپ را ببندد - چنین چیزی در
 * API نیست. تنها اهرمِ موجود input_field_placeholder است: متنی کمرنگ
 * داخلِ همان کادر، که پیش از تایپ دیده می‌شود.
 *
 * سه انگشت و نه یک جمله: خودِ پیام همین بالا گفته که باید دکمه را زد،
 * و این فقط باید چشم را به پایین ببرد. جمله‌ی دوم در کادرِ تایپ، همان
 * حرف را برای بار سوم می‌گفت.
 *
 * فرم شماره‌ی تایپ‌شده را به هر حال رد می‌کند؛ این نشانه فقط باعث
 * می‌شود کاربر پیش از تایپ کردن مکث کند، نه بعد از آن.
 *
 * سقفش ۶۴ کاراکتر است و متنِ بلندتر کلِ پیام را از سمتِ تلگرام رد
 * می‌کند، نه اینکه بریده شود.
 */
