...
// بالای فهرست فقط متن خودِ بخش می‌آید - نه عنوان ساختگی، نه شمارش.
//
// عنوان و شمارشِ خودکار حذف شدند چون متنی که آکادمی می‌نویسد خودش با
// یک تیتر شروع می‌شود و «۱۹ جلسه در آرشیو» بینشان می‌افتاد. یک سربرگ
// که با متن زیرش رقابت کند، بدتر از نداشتنش است.
//
// شمارش برای مدیر هنوز لازم است، پس به همان خط حالت مدیر رفته - جایی
// که کاربر نمی‌بیندش.
//
// نکته‌ی حالت مدیر جای متن را نمی‌گیرد، زیرش می‌نشیند: نسخه‌ی قبلی یکی
// را به‌جای دیگری می‌گذاشت و مدیر متنی را که خودش نوشته بود هرگز
// نمی‌دید - از دید او ویرایش «کار نمی‌کرد».
function adminHint(count) {
  return "⚙️ حالت مدیر — " + fa(count) + " مورد. کنار هر کدام دکمه‌ی مدیریت هست؛ این خط و آن دکمه را فقط شما می‌بینید.";
}

function listText(section, count, admin = false, intro = "") {
  const lines = [];
  if (intro) lines.push(intro);
  if (admin) lines.push("", adminHint(count));
  // اگر متن بخش پاک شده باشد، فهرست نباید بی‌سر بماند.
  return lines.join("\n").trim() || section.title;
}
